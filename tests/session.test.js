...
test('session authentication, expiry, permissions, audit and logout',async()=>{
 ...
 const auth=createAuth({logFile:path.join(dir,'events'),now:()=>time,idleMs:100,maxMs:250,authenticate:(email,password)=>password==='valid'?{email,nombre:'Test',rol:email==='admin'?'admin':'user',modulos:email==='denied'?[]:email==='ia'?['autoatencion-ia']:['gantt']}:null});
